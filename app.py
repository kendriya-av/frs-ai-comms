...
sample_data = load_combined_data(include_fomc_feed=False, include_news_feeds=False)   # GitHub + local data only at startup
            ...
            dcc.Link(
                html.Div([
                    ...
                    html.Span(name, style={"fontSize": "14px", "fontWeight": "500"}),
                          ...
                          "color": "rgba(255,255,255,0.4)" if href in ("/generate", "/scoring", "/faq") else "rgba(255,255,255,0.85)",
        ...
        dbc.Tabs([
            ...
            dbc.Tab(label="FOMC Statements", tab_style={"fontWeight": "500"}, children=[
                ...
                dbc.Row([
                    dbc.Col(card([
                        label("Count (up to)"),
                        dcc.Slider(id="fomc-limit", min=5, max=15, step=5, value=10,
            ...
            dbc.Tab(label="Press Releases", tab_style={"fontWeight": "500"}, children=[
                ...
                dbc.Row([
                    dbc.Col(card([
                        label("Count (up to)"),
                        dcc.Slider(id="press-limit", min=5, max=15, step=5, value=10,
                                   marks={5:"5", 10:"10", 15:"15"}),
                        html.Br(),
            ...
            dbc.Tab(label="Fed Speeches", tab_style={"fontWeight": "500"}, children=[
                ...
                dbc.Row([
                    dbc.Col(card([
                        label("Count (up to)"),
                        dcc.Slider(id="speech-limit", min=5, max=15, step=5, value=10,
                                   marks={5:"5", 10:"10", 15:"15"}),
                        html.Br(),
            ...
            dbc.Tab(label="News Outlets", tab_style={"fontWeight": "500"}, children=[
                ...
                dbc.Row([
                    dbc.Col(card([
                        ...
                        html.Br(),
                        label("Articles per outlet (up to)"),
                        dcc.Slider(id="news-limit", min=3, max=10, step=1, value=5,
...
def cb_press(n, limit):
    ...
    current_sources = sample_data.get("data_sources", [])
    current_sources = [s for s in current_sources if not s.startswith("Press Releases")]
    if count > 0:
        current_sources.append(src_label)
        sample_data["data_sources"] = current_sources
    return html.Div([
...
def cb_speech(n, limit):
    ...
    current_sources = sample_data.get("data_sources", [])
    current_sources = [s for s in current_sources if not s.startswith("Speeches")]
    if count > 0:
        current_sources.append(src_label)
        sample_data["data_sources"] = current_sources
    return html.Div([
...
def cb_news(n, selected_feeds, limit):
    ...
    src_label = f"News RSS ({len(valid)} articles from {len(sources)} outlets)"
    current_sources = sample_data.get("data_sources", [])
    current_sources = [s for s in current_sources if not s.startswith("News RSS")]
    current_sources.append(src_label)
    sample_data["data_sources"] = current_sources
...
def run_generation(n, data_type, count, batch_size, date_start, date_end, topics, load_after, model_store, cur_sig):
    ...
    try:
        ...
        if load_after and "yes" in load_after:
            ...
            # Update data_sources to reflect generation
            current_sources = sample_data.get("data_sources", [])
            gen_label = f"Generated: {len(records)} {data_type.replace('_', ' ')}"
            if gen_label not in current_sources:
                current_sources.append(gen_label)
                sample_data["data_sources"] = current_sources
            ...
            dash_table.DataTable(
                data=[
                    ...
                    {"Component": "Sentiment & Risk Reports", "Manual (hrs/mo)": f"{report_hrs_mo:.1f}",
                     ...
                     "AI Cost/mo": f"$0.00"},
                    {"Component": "Amazon Bedrock (Claude)", "Manual (hrs/mo)": "—",
                     "AI (hrs/mo)": "—", "Manual Cost/mo": "—",
                     "AI Cost/mo": f"${bedrock_mo:,.2f}"},
                    {"Component": "Amazon Comprehend (NLP)", "Manual (hrs/mo)": "—",
                     "AI (hrs/mo)": "—", "Manual Cost/mo": "—",
...
# ── Page: FAQ & Help ──────────────────────────────────────────────────────────
def faq_page():
    """Comprehensive FAQ highlighting every page, how sections work, and how values are calculated."""
    ...
    return html.Div([
        ...
        _faq_section("📥", "Inquiry & Response", [
            ("What is the Inquiry Queue?",
             ...
             "category badge, and priority badge. Click any inquiry to see its full detail and AI-generated draft response."),
        ...
        _faq_section("⚠️", "Risk Detector", [
            ("How does risk detection work?",
             ...
             "risk report with severity levels and recommended actions."),
        ...
        _faq_section("💰", "ROI Calculator", [
            ...
            ("What are the AWS cost components?",
             ...
             "These are real AWS pricing defaults that can be adjusted."),
        ...
        _faq_section("📂", "Upload Data", [
            ("What file formats are supported?",
             "JSON and CSV files. The data should contain records matching the expected schema for the selected data type "
             "(inquiries, social_media, news_articles, or response_templates)."),
            ("How does uploaded data integrate?",
             ...
             "existing data. All pages (Inquiry Queue, Sentiment Monitor, Insights, etc.) automatically reflect the new data."),
        ]),

        # ── Audit Log ─────────────────────────────────────────────────────
        _faq_section("📋", "Audit Log", [
            ("What is logged?",
             "Every AI action: draft responses, insights reports, risk detections, and classifications. Each entry records "
             "timestamp, action type, model ID used, input summary, and output summary for full traceability."),
            ("Where is the log stored?",
             "In a local JSON file (data/audit_log.json). In production, this would integrate with AWS CloudWatch Logs."),
        ]),

        # ── Trust & Safety ────────────────────────────────────────────────
        _faq_section("🛡️", "Trust & Safety", [
            ("What does the Responsible AI Posture show?",
             "Live AWS service status (Bedrock, Comprehend, S3, CloudWatch, Secrets Manager), classification confidence "
             "distribution, active guardrails, and audit trail health metrics."),
...
def handle_upload(contents, filename, data_type, existing_store):
    ...
    try:
        ...
        if not existing.empty and "source_file" in existing.columns:
            already = existing[existing["source_file"] == filename]
            if len(already) >= len(df):
                status = html.Div([
                    html.Span("ℹ ", style={"color": COLORS["warning"], "fontWeight": "700"}),
                    html.Span(f"Data from '{filename}' is already present ({len(already)} records in {data_type.replace('_', ' ')}). No duplicates added.",
                ...
                preview = dash_table.DataTable(
                    data=preview_df[preview_cols].to_dict("records"),
                    columns=[{"name": c.replace("_", " ").title(), "id": c} for c in preview_cols],
                    **TABLE_STYLE,
                )
                return status, preview, dash.no_update, dash.no_update
...
def refresh_live_data(n):
    global sample_data
    try:
        sample_data = load_combined_data(existing_data=sample_data, include_fomc_feed=False, include_news_feeds=False)
        sources     = sample_data.get("data_sources", [])
    ...
    return html.Div([
        ...
        card([
            ...
            dcc.Markdown(report, style={"fontSize": "14px", "lineHeight": "1.8",
                                        "color": COLORS["text"]}),
            html.Br(),
            # Store the raw report for download callbacks
            dcc.Store(id="insights-report-store", data=report),
            html.P("Download Report", style={"fontWeight": "600", "fontSize": "13px",
                                              "color": COLORS["navy"], "marginBottom": "8px"}),
            html.Div([
                html.Button("⬇ Markdown (.md)", id="dl-report-md", style={
                    "backgroundColor": "transparent", "color": COLORS["navy"],
                    "border": f"1px solid {COLORS['navy']}", "borderRadius": "6px",
                    "padding": "6px 14px", "fontWeight": "600", "fontSize": "12px",
                    "cursor": "pointer", "fontFamily": "'Inter', sans-serif", "marginRight": "8px",
                }),
                html.Button("⬇ HTML (.html)", id="dl-report-html", style={
                    "backgroundColor": "transparent", "color": COLORS["navy"],
                    "border": f"1px solid {COLORS['navy']}", "borderRadius": "6px",
                    "padding": "6px 14px", "fontWeight": "600", "fontSize": "12px",
                    "cursor": "pointer", "fontFamily": "'Inter', sans-serif", "marginRight": "8px",
                }),
                html.Button("⬇ Word (.docx)", id="dl-report-docx", style={
                    "backgroundColor": "transparent", "color": COLORS["navy"],
                    "border": f"1px solid {COLORS['navy']}", "borderRadius": "6px",
                    "padding": "6px 14px", "fontWeight": "600", "fontSize": "12px",
                    "cursor": "pointer", "fontFamily": "'Inter', sans-serif", "marginRight": "8px",
                }),
                html.Button("⬇ PDF (print)", id="dl-report-pdf", style={
                    "backgroundColor": "transparent", "color": COLORS["navy"],
                    "border": f"1px solid {COLORS['navy']}", "borderRadius": "6px",
                    "padding": "6px 14px", "fontWeight": "600", "fontSize": "12px",
                    "cursor": "pointer", "fontFamily": "'Inter', sans-serif",
...
# ── Callback: Download report in multiple formats ─────────────────────────────
@app.callback(
    Output("download-report", "data"),
    Input("dl-report-md",   "n_clicks"),
    Input("dl-report-html", "n_clicks"),
    Input("dl-report-docx", "n_clicks"),
    Input("dl-report-pdf",  "n_clicks"),
    State("insights-report-store", "data"),
    prevent_initial_call=True,
)
def download_report(n_md, n_html, n_docx, n_pdf, report):
    from dash import ctx
    if not report:
        return dash.no_update
    ...
    if triggered == "dl-report-md":
        return dcc.send_string(report, f"insights_report_{ts}.md")

    if triggered == "dl-report-html":
        import markdown as md_lib
        html_body = md_lib.markdown(report, extensions=["tables", "fenced_code"])
        html_doc = f"""<!DOCTYPE html>
<html><head><meta charset="utf-8">
<title>FRBSF Insights Report</title>
<style>
body {{ font-family: 'Segoe UI', Arial, sans-serif; max-width: 900px; margin: 40px auto;
       padding: 0 20px; color: #1B2A4A; line-height: 1.8; }}
...
<h1>FRBSF AI Communications — Insights Report</h1>
<p style="color:#6B7A99; font-size:13px;">Generated: {datetime.now().strftime("%B %d, %Y %H:%M")}</p>
<hr>
{html_body}
<hr><p style="color:#6B7A99; font-size:11px;">Federal Reserve Bank of San Francisco · AI Communications Intelligence System</p>
</body></html>"""
        return dcc.send_string(html_doc, f"insights_report_{ts}.html")

    if triggered == "dl-report-docx":
        import io
        ...
        # Title
        title_p = doc.add_heading("FRBSF AI Communications — Insights Report", level=1)
        ...
        # Footer
        doc.add_paragraph("")
        footer = doc.add_paragraph("Federal Reserve Bank of San Francisco · AI Communications Intelligence System")
        ...
        doc.save(buf)
        buf.seek(0)
        return dcc.send_bytes(buf.getvalue(), f"insights_report_{ts}.docx")

    if triggered == "dl-report-pdf":
        # Generate a print-optimized HTML that the user can print to PDF
        import markdown as md_lib
        html_body = md_lib.markdown(report, extensions=["tables", "fenced_code"])
...
body {{ font-family: 'Segoe UI', Arial, sans-serif; max-width: 800px; margin: 0 auto;
       padding: 40px 20px; color: #1B2A4A; line-height: 1.8; font-size: 12pt; }}
h1, h2, h3 {{ color: #0A1628; page-break-after: avoid; }}
table {{ border-collapse: collapse; width: 100%; margin: 16px 0; }}
th, td {{ border: 1px solid #ddd; padding: 8px 12px; text-align: left; font-size: 11pt; }}
th {{ background-color: #0A1628; color: white; -webkit-print-color-adjust: exact; }}
tr:nth-child(even) {{ background-color: #f8f9fa; -webkit-print-color-adjust: exact; }}
</style></head><body>
<h1>FRBSF AI Communications — Insights Report</h1>
<p style="color:#6B7A99;">Generated: {datetime.now().strftime("%B %d, %Y %H:%M")}</p>
<hr>
{html_body}
<hr><p style="color:#6B7A99; font-size:9pt; text-align:center;">
Federal Reserve Bank of San Francisco · AI Communications Intelligence System</p>
<script>window.onload = function() {{ window.print(); }}</script>
</body></html>"""
        return dcc.send_string(pdf_html, f"insights_report_{ts}_print.html")
